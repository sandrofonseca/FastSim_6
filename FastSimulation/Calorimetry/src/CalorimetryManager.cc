...
void CalorimetryManager::MuonMipSimulation(const FSimTrack& myTrack)
{
  ...
  int ifirstHcal=-1;
  int ilastEcal=-1;
  // Muon momentum  before of ECAL
  double eGen  = myTrack.ecalEntrance().e();
  ...
  //Implementation of Muon Brem in ECAL
        if (muonBremECAL){
...
///////////////////////////////////////////////////////////////////
    if(muonBremECAL->deltaP_BremPhoton().E()>0.){ LogDebug("FastCalorimetry") << "Muon Brem energy on ECAL: "<< energy << std::endl; }
           if( debug_ ) {
    ...
    }//end of the Muon Brem
    
 }//close the material effects here
...
////////////////////////////////////////////////////////////////////////////
  ...
  // Muon information in ECAL exit
 //Sandro Fonseca de Souza and Dilson de Jesus Damiao Nov 2012
 double e_ecal_exit  = myTrack.hcalEntrance().e();

 //Determine the position of the track at the ECAL exit.(Sandro Fonseca and Dilson de Jesus)
 RawParticle myPart= myTrack.ecalEntrance() * e_ecal_exit/eGen;
 myPart.setVertex(myPart.vertex());
//http://cmssdt.cern.ch/SDT/lxr/source/FastSimulation/BaseParticlePropagator/src/BaseParticlePropagator.cc?v=CMSSW_6_1_0_pre4#49
 BaseParticlePropagator myProp(myPart,129.0,320.9, getCalorimeter()->magneticField()); // this the field at (0,0)
 myProp.propagateToHcalEntrance();


  double pi = 0.5;//half of Helix radius
  int onHCAL = myTrack.onHcal();
 //HCAL entrance in Rz and Z
  double RZ_initial = 177.5;
  double Z_initial  = 335.0;
   //HCAL steps 
  double RZ_step = 17.280;
  double Z_step = 43.812;
   ...
   for(unsigned iseg=ifirstHcal;iseg<nsegments;++iseg)
	...
	if(segments[iseg].material()==CaloSegment::HCAL) {
	  ...
	  if (segmentSizeinX0>0.001) {
	    ...
	    theMuon.setID(-(int)charge*13);
            //HCAL splits
           for(int step_=0;step_<6;++step_){
            myProp.setPropagationConditions(RZ_initial,Z_initial);
          // bool done = myProp.propagate();

           LogDebug("FastCalorimetry") <<" RZ_initial: "<< RZ_initial<<" Z_initial: "<< Z_initial <<" step: "<< step_<<std::endl;
           r_ = RZ_initial + RZ_step;
           z_ = Z_initial + Z_step;   
           ...
           //Implementation of Muon Brem in HCAL     
            if (muonBremHCAL){
...
///////////////////////////////////////////////////////
    ...
    }//close switch using Muon Brem in step1

            myHcalHitMaker.setSpotEnergy(mipenergy);
  //          myHcalHitMaker.addHit(segments[iseg].entrance());
            myHcalHitMaker.addHit(myProp.helixRadius()*pi, myProp.helixStartPhi(),segments[iseg].material()==CaloSegment::HCAL);

//end the energy loss and muon brem at HCAL
 }//closing HCAL splits
      
////////////////////////////////////////////////////////
      }//end if of segments

     }//end if of the segments at HCAL
  }//end loop of the segments 

}//end if using first step in hcal
